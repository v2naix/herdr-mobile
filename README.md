...
- `ios/HerdrMobile/` 提供正式的 iOS 26 SwiftUI 客户端：配置单个 HTTPS Mac、用独立原生 bearer 会话验证、以设备密码绑定的 Keychain 保存 bootstrap token，并可浏览 agent pane；详情支持跟随最新输出、冻结历史阅读、原始宽度横向查看，以及等待服务端确认、保留失败草稿且不会自动重发的固定回复操作。
