...
经更新后的 #9 和 #15 发布要求确认，以下场景不再是 0.1.0 的真机发布门槛：无设备密码 Keychain、重装/备份迁移、重复显式认证拒绝，以及受控 TLS/协议/backend/herdr 不可用。实现的自动化覆盖和安全行为要求保持不变。
...
开始：2026-07-18______________ 结束：2026-07-18
失败项编号及非敏感说明：无。已从 0.1.0 真机发布门槛移除的场景见“0.1.0 发布范围调整”；Safari PWA 已通过。
```
