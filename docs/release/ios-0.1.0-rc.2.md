...
开始：2026-07-18______________ 结束：2026-07-18______________
失败项编号及非敏感说明：退出登录确认后未清除本地访问；Safari PWA 及其余未逐项报告的场景尚未验证。
```

## RC.2 真机结论

目标设备报告 Xcode 构建、安装、冷启动、正确 token 登录、pane 列表与详情、reader 滚动及返回底部、Reply 草稿、单次无破坏性命令和前后台恢复通过；错误 token 拒绝未验证，退出登录失败。

最小化确认 SwiftUI 关闭确认框时可能先将 presentation binding 设为 false，清除待确认动作；随后异步确认回调因动作已清除而不执行。回归检查覆盖该事件顺序，修复由确认按钮显式捕获已展示的动作。该修复会改变候选代码，因此 **RC.2 失败且不再作为验收输入**；必须创建新的候选引用并刷新受影响证据。
